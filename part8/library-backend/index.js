...
const resolvers = {
  Query: {
    allBooks: (root, args) => {
      let result = args.author ? books.filter((b) => b.author === args.author) : books
      result = args.genre ? result.filter((r) => r.genres.includes(args.genre)) : result
      return result
    },
    allAuthors: () => {
    ...
    //   return authors.filter(function (element, index) {
  ...
  Mutation: {
    addBook: (root, args) => {
      if (books.find((b) => b.title === args.title)) {
        throw new UserInputError('Book names must be unique', {
